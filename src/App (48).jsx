...
function LoginPage({onLogin,blockedMsg,onClearBlocked}) {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      // Busca perfil — usa maybeSingle() para não lançar erro se não existir
      const {data:p} = await supabase.from('profiles').select('*').eq('id',data.user.id).maybeSingle()

      // Só bloqueia se explicitamente desativado (active === false)
      // null/undefined = considera ativo (novo usuário ainda sem campo active)
      if(p && p.active === false && !p.is_super_admin) {
        await supabase.auth.signOut()
        setError('Seu acesso foi desativado. Contate o administrador.')
        return
      }

      // Verifica empresa só se tiver company_id E o perfil retornou
      if(p && p.company_id && !p.is_super_admin) {
        const {data:co} = await supabase.from('companies').select('active').eq('id',p.company_id).maybeSingle()
        // Só bloqueia se empresa existir E estiver explicitamente inativa
      ...
      setError('E-mail ou senha inválidos. (' + (err?.message||'erro desconhecido') + ')')
    } finally {
      setLoading(false)
    }
  }
...
export default function App() {
  ...
  useEffect(() => {
    ...
    // 1. Verifica sessão existente ao carregar a página
    supabase.auth.getSession().then(async ({ data: { session } }) => {
      if (session?.user) {
        ...
        try {
          ...
          if (p) {
            ...
            // Usuário autenticado mas sem perfil ainda — manda pro dash e tenta de novo em 2s
            setPage('dash')
            setTimeout(async () => {
              const p2 = await loadProfile(session.user.id)
              if (p2) { setProfile(p2); setPage(p2.is_super_admin ? 'super_dash' : 'dash') }
            }, 2000)
          }
        } catch (e) {
      ...
      resolved = true; clearTimeout(safetyTimer); setAuthLoading(false)
    })
      ...
      if (event === 'SIGNED_IN' && session?.user) {
        ...
        try {
          ...
          if (p) {
            ...
            setPage('dash')
            // Retry após 2s — perfil pode ainda estar sendo criado
            setTimeout(async () => {
              const p2 = await loadProfile(session.user.id)
              if (p2) { setProfile(p2); setPage(p2.is_super_admin ? 'super_dash' : 'dash') }
            }, 2000)
          ...
          console.error('onAuthStateChange loadProfile error:', e?.message)
          setPage('dash')
        }
        resolved = true; clearTimeout(safetyTimer); setAuthLoading(false)
      } else if (event === 'SIGNED_OUT') {
        setUser(null); setProfile(null); setCompanyName(''); setPage(null)
        resolved = true; clearTimeout(safetyTimer); setAuthLoading(false)
      }
      // Ignora outros eventos (TOKEN_REFRESHED, INITIAL_SESSION, etc) para não sobrescrever profile
    })
  ...
  const handleLogin = async (loggedUser, loggedProfile) => {
    setUser(loggedUser)
    setProfile(loggedProfile || null)
    setPage(loggedProfile?.is_super_admin ? 'super_dash' : 'dash')
    if (loggedProfile?.company_id) {
      try {
        const { data: c } = await supabase.from('companies').select('name').eq('id', loggedProfile.company_id).single()
        setCompanyName(c?.name || '')
      } catch (e) {
        setCompanyName('')
      }
