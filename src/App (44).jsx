...
function LoginPage({onLogin,blockedMsg,onClearBlocked}) {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      const {data:p, error:profileErr} = await supabase.from('profiles').select('*').eq('id',data.user.id).single()
      if(profileErr) {
        setError('[DIAG] Erro ao buscar profile: ' + profileErr.message + ' | code: ' + (profileErr.code||'?'))
    ...
    supabase.auth.getSession().then(async ({ data: { session } }) => {
      if (session?.user) {
        ...
        try {
          ...
          console.error('❌ loadProfile error (getSession):', e?.message || e)
          // Tenta buscar sem RLS como fallback (usando maybeSingle para não lançar erro em 0 rows)
          try {
            const { data: p2 } = await supabase.from('profiles').select('*').eq('id', session.user.id).maybeSingle()
            if(p2) {
              setProfile(p2)
              setPage(p2.is_super_admin ? 'super_dash' : 'dash')
            } else {
              setProfile(null)
              setPage('dash')
            }
          } catch(e2) {
            console.error('❌ fallback profile error:', e2?.message || e2)
            setProfile(null)
            setPage('dash')
          }
        }
      ...
      console.error('❌ getSession catch:', e?.message || e)
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(async (_event, session) => {
      ...
      if (session?.user) {
        try {
          ...
          console.error('❌ loadProfile error (onAuthStateChange):', e?.message || e)
          try {
            const { data: p2 } = await supabase.from('profiles').select('*').eq('id', session.user.id).maybeSingle()
            if(p2) {
              setProfile(p2)
            ...
            console.error('❌ fallback profile error (auth change):', e2?.message || e2)
            setProfile(null)
            setPage('dash')
          }
        }
