...
function Card({children,style,onClick}) {
  return <div onClick={onClick} style={{background:C.white,border:`1px solid ${C.border}`,borderRadius:14,padding:'14px 16px',...style}}>{children}</div>
}
...
function SectionHeader({title,sub,action}) {
  return (
    <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:16}}>
      <div>
        <h2 style={{fontSize:17,fontWeight:800,color:C.navy,margin:0}}>{title}</h2>
  ...
  const uploadCert = async()=>{
    ...
    try {
      ...
      reader.onload = async(e)=>{
        ...
        const resp = await fetch(`https://homologacao.focusnfe.com.br/v2/empresas/${cnpjLimpo}/certificado`,{
          method:'POST',
    ...
    <Modal title={isNew?'Nova Empresa':`Editar — ${company.name}`} onClose={onClose} maxWidth={500}>
      ...
      {tab==='fiscal'&&(
        <div style={{display:'flex',flexDirection:'column',gap:12}}>
          ...
          <div style={{background:C.infoPale,border:`1.5px solid ${C.info}`,borderRadius:10,padding:'12px 14px'}}>
...
function SuperUsers() {
  ...
  const emptyForm = {email:'',password:'',name:'',company_id:'',role:'ADMIN'}
  ...
  const save = async () => {
    if(!form.email||!form.password||!form.company_id){toast.show('Preencha todos os campos','error');return}
    setSaving(true)

    // Cria usuário via SQL (não derruba sessão do super admin)
    const {data:newUserId,error:authErr} = await supabase.rpc('create_auth_user',{
      user_email: form.email.trim(),
      user_password: form.password,
      user_name: form.name.trim()||form.email.split('@')[0],
    })
    if(authErr){
      const msg = authErr.message?.includes('duplicate')||authErr.message?.includes('already')
        ? 'E-mail já cadastrado.'
        : 'Erro: '+authErr.message
      toast.show(msg,'error');setSaving(false);return
    }
    if(!newUserId){toast.show('Erro: ID não retornado','error');setSaving(false);return}

    const allMods = ALL_MODULES.map(m=>m.id)
    const {error:profErr} = await supabase.rpc('create_user_profile',{
      user_id: newUserId,
      user_email: form.email.trim(),
      user_name: form.name.trim()||form.email.split('@')[0],
      user_company_id: form.company_id,
      user_role: form.role,
      user_modules: allMods,
    })
    setSaving(false)
    if(profErr){toast.show('Erro no perfil: '+profErr.message,'error');return}
...
function PageDash({user,companyId,setPage,setStockFilter,setSalesFilter}) {
  ...
  useEffect(()=>{
    ...
    const loadDash = async () => {
      ...
      try {
        ...
        let qp = supabase.from('products').select('id,stock_quantity,min_stock,price,cost_price',{count:'exact'})
        if(companyId) qp = qp.eq('company_id',companyId)
        const {data:prods,count:prodCount} = await qp
        if(!isMounted) return;
        const stockAlerts = (prods||[]).filter(p=>(p.stock_quantity||0)<=(p.min_stock||5)).length
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',gap:14}}>
      ...
      {/* ── MÉTRICAS GRID ── */}
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
        <MCard label="Faturamento" value={fmtBRL(stats.salesTotal)} sub={`${stats.salesCount} pedidos`} icon="💰" accent={C.navy} onClick={()=>{setSalesFilter({mode:'month',month:now.getMonth(),year:now.getFullYear()});setPage('sales')}}/>
...
function BatchImportModal({companyId, onClose, onDone, toast}) {
  ...
  const importAll = async() => {
    ...
    setSaving(true)
    const payload = rows.map(r=>({...r, active:true, company_id:companyId, updated_at:new Date().toISOString()}))
    let totalInserted = 0
    for(let i=0;i<payload.length;i+=50) {
      const {error} = await supabase.from('products').insert(payload.slice(i,i+50))
      if(error){toast.show('Erro: '+error.message,'error');setSaving(false);return}
      totalInserted += Math.min(50, payload.length-i)
    }
...
function PageProducts({companyId, stockFilter, setStockFilter}) {
  ...
  const emptyForm = {name:'',sku:'',category:'',price:'',cost:'',stock:'',min_stock:'5',unit:'un',ean:''}
  const [form,setForm] = useState(emptyForm)
  const toast = useToast()
  ...
  const filtered = useMemo(() => {
    let list = products.filter(p=>p.name?.toLowerCase().includes(deferredSearch.toLowerCase())||p.sku?.toLowerCase().includes(deferredSearch.toLowerCase()))
  ...
  const openNew = () => { setEditProduct(null); setForm(emptyForm); setShowModal(true) }

  const openEdit = (p) => {
    setEditProduct(p)
    setForm({name:p.name||'',sku:p.sku||'',category:p.category||'',price:p.price!=null?String(p.price):'',cost:p.cost_price!=null?String(p.cost_price):'',stock:p.stock_quantity!=null?String(p.stock_quantity):'',min_stock:p.min_stock!=null?String(p.min_stock):'5',unit:p.unit||'un',ean:p.ean||''})
    setShowModal(true)
  }
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',gap:16}}>
      ...
      <Input value={search} onChange={setSearch} placeholder="🔍 Buscar por nome ou SKU..."/>
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:8}}>
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',gap:16}}>
      ...
      {loading?<Spinner/>:products.map(p=>{
        const t=tag(p)
        return (
          <Card key={p.id} style={{display:'flex',alignItems:'center',gap:10,padding:'12px 14px'}}>
            <div style={{flex:1,minWidth:0}}>
              <p style={{fontSize:13,fontWeight:700,color:C.text,margin:0}}>{p.name}</p>
...
async function emitirNFCe({cart, total, payment, customer, fiscalConfig, companyId, saleRef}) {
...
function PagePDV({companyId}) {
  ...
  const finalize = async()=>{
    ...
    if(error){toast.show('Erro ao registrar venda','error');setSaving(false);return}
    ...
    setCart([]);setCustomer('');setSearch('');setInstallments(1);setDiscount('')
    setSuccess(true)
  }
  ...
  const handleNFCe = async() => {
    ...
    try {
      const result = await emitirNFCe({cart:successCart,total:successTotal,payment:successPayment,customer:successCustomer,fiscalConfig,companyId,saleRef:lastSaleRef})
      setNfceResult(result)
      toast.show('NFC-e emitida com sucesso!')
    } catch(err) {
      toast.show('Erro NFC-e: '+err.message,'error')
    }
      ...
      {cart.length>0&&(
        <div style={{background:C.white,borderRadius:16,border:`1px solid ${C.border}`,overflow:'hidden'}}>
          ...
          <div style={{padding:'12px 14px',display:'flex',flexDirection:'column',gap:10}}>
            ...
            <Input label="Cliente (opcional)" value={customer} onChange={setCustomer} placeholder="Nome do cliente"/>
            <button onClick={finalize} disabled={saving||!cart.length}
              style={{background:saving?C.muted:`linear-gradient(135deg,${C.green},${C.greenLight})`,color:'#fff',border:'none',borderRadius:13,padding:'15px',fontSize:14,fontWeight:900,cursor:saving?'not-allowed':'pointer',boxShadow:'0 4px 20px rgba(34,197,94,0.3)',transition:'all .15s'}}>
    ...
    <div style={{display:'flex',flexDirection:'column',gap:16}}>
      ...
      {cancelTarget&&(
        <Modal title="Cancelar Venda" onClose={()=>setCancelTarget(null)}>
          <div style={{display:'flex',flexDirection:'column',gap:14}}>
            <div style={{background:C.dangerPale,border:`1px solid #FCA5A5`,borderRadius:10,padding:'12px 14px'}}>
...
function PageAccounts({companyId,type}) {
  ...
  const [form,setForm] = useState({desc:'',value:'',due_date:'',category:''})
  const [saving,setSaving] = useState(false)
  const [deleting,setDeleting] = useState(false)
  const toast = useToast()
  ...
  const openNew = () => {
    setEditItem(null)
    setForm({desc:'',value:'',due_date:'',category:''})
    setShowModal(true)
  }
  const openEdit = (item) => {
    setEditItem(item)
    setForm({desc:item.description||'',value:String(item.value||''),due_date:item.due_date||'',category:item.category||''})
    setShowModal(true)
  }
  ...
  const save = async()=>{
    if(!form.desc||!form.value){toast.show('Preencha descrição e valor','error');return}
    setSaving(true)
    const payload = {company_id:companyId,description:form.desc,value:parseFloat(form.value),due_date:form.due_date||null,category:form.category||'Geral'}
    if(editItem) {
      ...
      if(error){toast.show('Erro ao salvar: '+error.message,'error');setSaving(false);return}
      toast.show('Lançamento salvo!')
    }
    setSaving(false)
    setShowModal(false);setEditItem(null);setForm({desc:'',value:'',due_date:'',category:''});fetchItems()
  }
  ...
  const pay = async(item)=>{
    const now = new Date()
    await supabase.from(table).update({status:'paid',paid_at:now.toISOString()}).eq('id',item.id)
          ...
          <Card key={item.id}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:8}}>
              <div style={{flex:1,minWidth:0}}>
                ...
                <p style={{fontSize:11,color:C.muted,margin:'3px 0 0'}}>{item.due_date?new Date(item.due_date+'T00:00:00').toLocaleDateString('pt-BR'):'Sem vencimento'} · {item.category||'Geral'}</p>
...
function PageUsers({companyId}) {
  ...
  const save = async()=>{
    if(!form.email||!form.password){toast.show('E-mail e senha obrigatórios','error');return}
    if(form.password.length<6){toast.show('Senha mínimo 6 caracteres','error');return}
    setSaving(true)

    // Cria o usuário via função SQL (não derruba a sessão do admin)
    const {data:newUserId,error:authErr} = await supabase.rpc('create_auth_user',{
      user_email: form.email.trim(),
      user_password: form.password,
      user_name: form.name.trim()||form.email.split('@')[0],
    ...
    if(!newUserId){toast.show('Erro: não foi possível criar o usuário','error');setSaving(false);return}

    const defaultPerms = ALL_MODULES.map(m=>m.id)
    const {error:profErr} = await supabase.rpc('create_user_profile',{
      user_id: newUserId,
      user_email: form.email.trim(),
      user_name: form.name.trim()||form.email.split('@')[0],
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',gap:16}}>
      ...
      <SectionHeader title="Usuários" sub={`${users.length} cadastrados`}
        action={<Btn size="sm" onClick={()=>setShowModal(true)}>+ Novo Usuário</Btn>}/>
...
function ModalEmpresa({onClose,companyId}) {
  ...
  useEffect(()=>{
    ...
    const load = async()=>{
      ...
      if(co) setForm({name:co.name||'',cnpj:co.cnpj||'',phone:co.phone||'',address:co.address||'',city:co.city||'',state:co.state||''})
      setLoading(false)
    }
    load()
  },[companyId])
...
function ModalPerfil({user,onClose}) {
  ...
  const save = async()=>{
    ...
    if(pe){toast.show('Erro: '+pe.message,'error');setSaving(false);return}
    if(newPass){
      const {error:ae} = await supabase.auth.updateUser({password:newPass})
      if(ae){toast.show('Erro na senha: '+ae.message,'error');setSaving(false);return}
    }
  ...
  const uploadCert = async()=>{
    ...
    try {
      ...
      reader.onload = async(e) => {
        ...
        const resp = await fetch(`${FOCUS_URL}/empresas/${form.cnpj?.replace(/\D/g,'')}/certificado`, {
          method:'POST',
...
export default function App() {
  ...
  const [salesFilter,setSalesFilter] = useState(null)  // {mode,month,year}

  const initialLoadDone = useRef(false)

  // ⚠️ useMemo DEVE ficar aqui, ANTES de qualquer return condicional.
  ...
  useEffect(() => {
    ...
    const loadProfile = async (userId) => {
      ...
      if(p.is_super_admin === true) return p
      // Bloqueia usuário inativo
      if(p.active === false) {
        return {blocked:'Seu acesso foi desativado. Contate o administrador.'}
      }
      if(p?.company_id && mounted) {
        const { data: co } = await supabase.from('companies').select('name,active').eq('id', p.company_id).single()
        if(mounted) {
          if(!co || co.active === false) {
            return {blocked:'O acesso desta empresa está suspenso. Contate o suporte.'}
          }
    ...
    const initAuth = async () => {
      try {
        ...
        if(mounted && session?.user) {
          ...
          if(!mounted) return
          if(!p || p.blocked) {
            // Faz signOut AQUI, fora do loadProfile, para não disparar onAuthStateChange cedo
            await supabase.auth.signOut()
            setUser(null); setProfile(null); setPage(null)
            if(p?.blocked && mounted) setLoginBlockedMsg(p.blocked)
            return
          }
      ...
      } catch(err) { console.error('Erro na auth inicial', err) }
      finally {
        if(mounted) {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(async (_event, session) => {
      if(!mounted) return
      // Ignora eventos durante carregamento inicial e renovação de token (evita loop de login)
      if(!initialLoadDone.current) return
      if(_event === 'INITIAL_SESSION' || _event === 'TOKEN_REFRESHED') return
      // Só atualiza user se houve mudança real (SIGNED_IN / SIGNED_OUT)
  ...
  // Realtime: atualiza permissões imediatamente quando admin editar o perfil
  useEffect(()=>{
    if(!user) return
    const channel = supabase
      ...
      .on('postgres_changes', {event:'UPDATE',schema:'public',table:'profiles',filter:`id=eq.${user.id}`},
        (payload) => {
          const d = payload.new
          if(!d) return
          setProfile(prev => prev ? {...prev, allowed_modules: d.allowed_modules, role: d.role, active: d.active} : prev)
        }
      )
      .subscribe()
  ...
  const logout = async()=>{
    // Desativa o listener temporariamente para não criar loop
  ...
  if(authLoading) return (
    <div style={{minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center',background:C.navy}}>
      <div style={{textAlign:'center'}}><DaliLogo size={56}/><p style={{color:'rgba(255,255,255,0.6)',fontSize:13,marginTop:16}}>Carregando...</p></div>
    </div>
  )

  if(!user) return <LoginPage onLogin={setUser} blockedMsg={loginBlockedMsg} onClearBlocked={()=>setLoginBlockedMsg('')}/>
  ...
  // Aplica filtro de módulos permitidos para não-admins
  const nav = isSuperAdmin
    ? NAV_SUPER
    : userRole === 'ADMIN'
      ? NAV_CLIENT // admin vê tudo
      : NAV_CLIENT.filter(n => allowedModules.includes(n.id))



  const pageTitles = Object.fromEntries([...NAV_CLIENT,...NAV_SUPER].map(n=>[n.id,n.label]))
  const props = {user,companyId}

  const renderPage = () => {
    // Guard: verifica se o usuário tem acesso ao módulo solicitado
    const superPages = ['super_dash','super_companies','super_licenses','super_users','super_settings']
    const isSuper = superPages.includes(page)
    if(!isSuperAdmin && !isSuper && page !== 'settings') {
      const moduleId = page  // os ids de página são iguais aos ids de módulo
      if(userRole !== 'ADMIN' && !allowedModules.includes(moduleId)) {
        return (
          <div style={{display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',minHeight:300,gap:12,padding:32,textAlign:'center'}}>
            <div style={{fontSize:48}}>🔒</div>
            <h2 style={{fontSize:16,fontWeight:800,color:C.navy,margin:0}}>Acesso Restrito</h2>
            <p style={{fontSize:13,color:C.muted,margin:0}}>Você não tem permissão para acessar este módulo.<br/>Contate o administrador.</p>
          </div>
        )
      }
    }
    switch(page) {
      ...
      case 'super_settings':  return <PageSettings user={user} companyId={companyId} onLogout={logout} isSuperAdmin={isSuperAdmin}/>
      case 'dash':            return <PageDash {...props} setPage={setPage} setStockFilter={setStockFilter} setSalesFilter={setSalesFilter}/>
      case 'products':        return <PageProducts {...props} stockFilter={stockFilter} setStockFilter={setStockFilter}/>
      case 'inventory':       return <PageInventory {...props}/>
      case 'pdv':             return <PagePDV {...props}/>
      case 'sales':           return <PageSales {...props} userRole={userRole} userEmail={userEmail} salesFilter={salesFilter}/>
      ...
      case 'settings':        return <PageSettings user={user} companyId={companyId} onLogout={logout} isSuperAdmin={isSuperAdmin}/>
  ...
  return (
    <div style={{display:'flex',minHeight:'100vh',background:C.surface}}>
      {!isMobile&&<Sidebar page={page} setPage={setPage} nav={nav} companyName={companyName} isSuperAdmin={isSuperAdmin}/>}
      <div style={{flex:1,display:'flex',flexDirection:'column',minWidth:0}}>
          ...
          {isMobile?(
            <>
              <div style={{display:'flex',alignItems:'center',gap:10}}>
                <div style={{width:30,height:30,background:C.navy,borderRadius:9,display:'flex',alignItems:'center',justifyContent:'center'}}><DaliLogo size={24}/></div>
            ...
            <div>
              ...
              <p style={{fontSize:11,color:C.muted,margin:0}}>{isSuperAdmin?'Super Admin · DALI Tech':`${companyName?companyName+' · ':''}DALI Tech ERP`}</p>
            </div>
          )}
          <div style={{display:'flex',alignItems:'center',gap:8}}>
        ...
        <div style={{flex:1,padding:isMobile?'16px 14px 96px':'24px 28px',overflowY:'auto'}}>
          <div style={{maxWidth:isMobile?'100%':760,margin:'0 auto'}}>{renderPage()}</div>
        </div>
      </div>
      {isMobile&&<BottomNav page={page} setPage={setPage} onMenu={()=>setDrawerOpen(true)} isSuperAdmin={isSuperAdmin}/>}
      {isMobile&&drawerOpen&&<MobileDrawer page={page} setPage={setPage} onClose={()=>setDrawerOpen(false)} nav={nav} companyName={companyName} isSuperAdmin={isSuperAdmin}/>}
    </div>
  )
}
