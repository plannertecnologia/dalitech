...
const PLANS = {
  ...
  pro: {
    ...
    maxProducts: 500,
    maxSalesMonth: 1000,
    modules: ['dash','products','inventory','pdv','sales','cashflow','payable','receivable','users','settings'],
    price: 'R$ 197/mês',
  },
  enterprise: {
    label: 'Enterprise',
    ...
    modules: ['dash','products','inventory','pdv','sales','cashflow','payable','receivable','users','settings','reports','fiscal'],
    price: 'R$ 397/mês',
  },
}
...
function usePlan(companyId) {
  ...
  const canAddUser = () => plan && !plan.inactive && usage.users < plan.maxUsers
  const canAddProduct = () => plan && !plan.inactive && usage.products < plan.maxProducts
  const canAddSale = () => plan && !plan.inactive && usage.salesMonth < plan.maxSalesMonth
  ...
  return (
    <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, background: C.white, borderTop: `1px solid ${C.border}`, display: 'flex', alignItems: 'center', zIndex: 100, paddingBottom: 'env(safe-area-inset-bottom,0px)' }}>
      {quick.map(item => {
        ...
        return (
          <button key={item.id} onClick={() => !locked && setPage(item.id)} style={{
            ...
            <span style={{ fontSize: 20 }}>{item.icon}</span>
            <span style={{ fontSize: 9, fontWeight: 700, color: page === item.id ? C.navy : C.subtle, textTransform: 'uppercase', letterSpacing: '0.04em' }}>
              {item.label}
            </span>
            {page === item.id && <div style={{ width: 18, height: 2.5, background: C.navy, borderRadius: 2 }} />}
            {locked && <span style={{ fontSize: 8, color: C.danger }}>🔒</span>}
          </button>
        )
      })}
      <button onClick={onMenu} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3, padding: '10px 4px 8px', border: 'none', background: 'transparent', cursor: 'pointer' }}>
...
export default function App() {
  ...
  useEffect(() => {
    supabase.auth.getSession().then(async ({ data: { session } }) => {
      if (session?.user) {
        ...
        try {
          const { data: p } = await supabase.from('profiles').select('*,companies(name)').eq('id', session.user.id).single()
          setProfile(p || null)
          setCompanyName(p?.companies?.name || '')
  ...
  const props = { user, companyId, canAddProduct, canAddSale, canAddUser, plan, usage }

  const renderPage = () => {
    // Verifica se módulo está bloqueado pelo plano
    if (!isSuperAdmin && plan && !canAccess(page)) {
      return <PlanBlock plan={plan} feature={pageTitles[page] || 'Este módulo'} />
    }
    switch (page) {
