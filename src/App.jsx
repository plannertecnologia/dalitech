  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(async ({ data: { session } }) => {
      if (session?.user) {
        ...
        try {
          ...
          console.log("[DIAG-1] profile:", p, "| error:", profileError, "| uid:", session.user.id, "| is_super_admin typeof:", typeof p?.is_super_admin, "| value:", p?.is_super_admin)
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(async (_event, session) => {
      ...
      if (session?.user) {
        try {
          const { data: p, error: profileError } = await supabase.from('profiles').select('*').eq('id', session.user.id).single()
          console.log("[DIAG-2] profile:", p, "| error:", profileError, "| uid:", session.user.id, "| is_super_admin typeof:", typeof p?.is_super_admin, "| value:", p?.is_super_admin)
          if (profileError) throw profileError
          setProfile(p || null)
          setPage(p?.is_super_admin ? 'super_dash' : 'dash')
          if (p?.company_id) {
