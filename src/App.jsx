...
function BatchImportModal({companyId, onClose, onDone, toast}) {
  ...
  const importAll = async() => {
    ...
    const payload = rows.map(r=>{
      const {_line, ...data} = r  // remove campo interno — não existe no banco
      return {
        ...data, active:true, company_id:companyId,
        updated_at:new Date().toISOString(),
        ean: (data.ean && validateGTIN13(data.ean)) ? data.ean : generateGTIN13(),
      }
    })
    ...
    for(let i=0;i<payload.length;i+=50) {
      ...
      if(error?.message?.includes('ean')) {
        ...
        if(e2){toast.show('Erro: '+e2.message,'error');setSaving(false);return}
      } else if(error){
        toast.show('Erro: '+error.message,'error');setSaving(false);return
      }
